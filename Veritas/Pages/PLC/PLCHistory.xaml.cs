...
namespace Veritas.Pages.PLC
{
    ...
    /// </summary>
    public sealed partial class PLCHistory : Page
    {
        ...
        public PLCHistory()
        {
            ...
            this.headerControl.ServerName = "Trollhättan PLC";
        ...
        {
            string xmlFile = XML + "?machine=3&from=" + from.ToString("yyyyMMddHH:mm") + "&to=" + to.ToString("yyyyMMddHH:mm");
            ...
            for (i = 0; i < machineNodes.Length; i++)
            {
                ...
                if (i == 0)
                    this.tempChart.SetLeftLabel(dateTime.ToString("HH:mm"));
                ...
                // Temp
                this.tempChart.AddDataPoints(new NameValueItem(dateTime, int.Parse(((XmlElement)machineNodes[i]).GetElementsByTagName("plcTemp").Item(0).InnerText)));

                if (!loadTimer)
                    if (this.tempChart.Count() == 30)
                        break;
            }
            ...
            if (i > 0)
            {
                ...
                this.tempChart.SetRightLabel(this.tempChart.GetLastItem().DateTime.ToString("HH:mm"));
            }
        }
